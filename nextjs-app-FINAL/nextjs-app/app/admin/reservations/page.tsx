...
export default function ReservationsPage() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const [res, props, sts] = await Promise.all([
        ...
        getGenericClient("reservationStatus", ROLE).findAll() as unknown as Promise<
  ...
  }, [reservations, filterPropertyId, filterStatusId, filterFrom, filterTo]);

  const calendarReservations = useMemo(
    () => reservations.filter((r) => r.property?.id === calendarPropertyId),
    [reservations, calendarPropertyId]
  );
