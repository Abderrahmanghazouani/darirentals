/**
 * Traductions FR/EN — solution maison (voir NOTES-multi-langue.md pour le choix technique).
 * "fr" est la référence de forme : "en" doit avoir EXACTEMENT les mêmes clés (TypeScript
 * lève une erreur de compilation sinon, via `satisfies Dict` plus bas).
 *
 * Ne contient QUE des libellés fixes de l'UI — jamais de données (noms de propriétés, montants,
 * contenu généré par l'IA...), qui restent tels quels quelle que soit la langue choisie.
 */

const fr = {
  common: {
    loading: "Chargement...",
    cancel: "Annuler",
    close: "Fermer",
  },
  // Sidebar + topbar (components/app-shell.tsx) et libellés de navigation des layouts admin/collaborateur.
  nav: {
    sectionOverview: "Vue d'ensemble",
    sectionOperations: "Opérations",
    sectionFinances: "Finances",
    sectionTeam: "Équipe",
    dashboard: "Tableau de bord",
    properties: "Propriétés",
    reservations: "Réservations",
    reservationRequests: "Demandes de réservation",
    tasks: "Tâches",
    charges: "Charges",
    payments: "Paiements",
    financialReports: "Rapports financiers",
    exchangeRates: "Taux de change",
    collaborators: "Collaborateurs",
    clients: "Clients",
    allModules: "Tous les modules",
    defaultAgency: "Agence principale",
    company: "Société",
    account: "Compte",
    navigation: "Navigation",
    openMenu: "Ouvrir le menu",
    expandMenu: "Déplier le menu",
    collapseMenu: "Replier le menu",
    expand: "Déplier",
    collapse: "Replier",
    logout: "Déconnexion",
    home: "Accueil",
    search: "Rechercher",
    notifications: "Notifications",
    breadcrumbProperty: "Biens & logements",
    breadcrumbProfitability: "Rentabilité",
  },
  reserver: {
    heroTitle: "Nos logements disponibles",
    heroSubtitle: "Choisissez un logement et envoyez-nous votre demande — nous vous répondons rapidement.",
    displayPricesIn: "Afficher les prix en :",
    loadingProperties: "Chargement...",
    noProperties: "Aucun logement disponible pour le moment.",
    perNight: "/ nuit",
    people: "personnes",
    viewPosition: "Voir la position",
    request: "Demander",
    requestForPrefix: "Demande pour :",
    requestSentTitle: "Demande envoyée",
    thanksTitle: "Merci !",
    requestSentBody: "Votre demande a bien été envoyée. Nous vous recontactons rapidement au",
    checkIn: "Arrivée",
    checkOut: "Départ",
    fullName: "Nom complet",
    phone: "Téléphone",
    messageOptional: "Message (optionnel)",
    minimumFieldsError: "Renseigne au moins ton nom et ton téléphone.",
    genericSubmitError: "Erreur lors de l'envoi",
    sendRequest: "Envoyer la demande",
    sending: "Envoi...",
  },
  login: {
    title: "Connexion",
    username: "Nom d'utilisateur",
    password: "Mot de passe",
    submit: "Se connecter",
    submitting: "Connexion...",
    genericError: "Échec de la connexion",
    unrecognizedRoleError: "Rôle non reconnu pour cet utilisateur.",
    // Page de connexion redesignée (feature/style-frontend) : volet branding + formulaire.
    brandBadge: "Gestion locative nouvelle génération",
    brandTitleLine1: "Toute votre gestion",
    brandTitleLine2: "au même endroit.",
    brandSubtitle:
      "Retrouvez vos propriétés, réservations, charges, paiements et équipes dans un espace conçu pour simplifier votre quotidien.",
    benefitMultiCompany: "Gérez plusieurs sociétés depuis un seul compte",
    benefitIsolation: "Gardez vos données strictement cloisonnées",
    benefitRealtime: "Suivez votre activité et votre rentabilité en temps réel",
    brandFooter: "Un espace de gestion pensé pour les professionnels de la location.",
    backHome: "Retour à l'accueil",
    backHomeShort: "Accueil",
    welcomeTitle: "Bienvenue",
    welcomeSubtitle: "Connectez-vous à votre espace DariRentals pour continuer.",
    usernamePlaceholder: "Votre nom d'utilisateur",
    passwordPlaceholder: "Votre mot de passe",
    showPassword: "Afficher le mot de passe",
    hidePassword: "Masquer le mot de passe",
    securityNote:
      "Votre accès est protégé. Les informations de votre espace restent séparées selon votre société et votre rôle.",
  },
  dashboardHeader: {
    title: "Tableau de bord",
    activeProperty: "propriété active",
    activeProperties: "propriétés actives",
    account: "Compte",
    logout: "Déconnexion",
  },
  dashboardStats: {
    properties: "Propriétés",
    active: "active(s)",
    reservations: "Réservations",
    upcoming: "à venir",
    clients: "Clients",
    revenue: "Revenu (réservations)",
  },
  // Dashboard "Vue d'ensemble" redesigné (feature/style-frontend) - admin & collaborateur.
  dashboardHome: {
    greeting: "Bonjour",
    greetingReturning: "ravi de vous revoir.",
    subtitle: "Voici ce qui se passe dans votre portefeuille aujourd'hui.",
    subtitleCompanyPrefix: "Voici ce qui se passe chez",
    subtitleCompanySuffix: "aujourd'hui.",
    subtitleCompanyFallback: "votre société",
    scanInvoice: "Scanner une facture",
    statRevenue: "Chiffre d'affaires",
    statRevenueHint: "Hors réservations annulées",
    statOccupancy: "Taux d'occupation",
    statReservations: "Réservations",
    upcomingSuffix: "à venir",
    statNetRevenue: "Revenu net",
    statNetRevenueHint: "Revenus − charges",
    todoTitle: "À faire aujourd'hui",
    todoSubtitle: "Les tâches qui nécessitent votre attention",
    todoEmpty: "Rien d'urgent aujourd'hui.",
    taskFallback: "Tâche",
    seeAllTasks: "Voir toutes les tâches",
    recentReservationsTitle: "Réservations récentes",
    recentReservationsSubtitle: "Les dernières réservations de vos logements",
    seeAll: "Voir toutes",
    colProperty: "Logement",
    colClient: "Client",
    colDates: "Dates",
    colAmount: "Montant",
    colStatus: "Statut",
    noReservations: "Aucune réservation.",
    calendarTitle: "Calendrier",
    calendarSubtitle: "Occupation des logements",
  },
  propertyMap: {
    legendActive: "Occupée actuellement",
    legendEndingSoon: "Départ sous 3 jours",
    legendIdle: "Libre",
    viewProfitability: "Voir la rentabilité",
    viewProperty: "Voir la propriété",
    emptyTitle: "Aucune propriété localisée",
    emptySubtitle: "Ajoutez une latitude/longitude à vos propriétés pour les voir ici.",
  },
  dayTimeline: {
    title: "Aujourd'hui",
    subtitle: "Arrivées, départs et tâches du jour",
    emptyTitle: "Rien de prévu aujourd'hui",
    emptySubtitle: "Profitez-en pour souffler un peu.",
    arrivalLabel: "Arrivée",
    departureLabel: "Départ",
    taskLabel: "Tâche à échéance",
  },
  healthScore: {
    title: "Santé du portefeuille",
  },
  revenueIntelligence: {
    title: "Revenus",
    newBadge: "Nouveau",
    currentMonthRevenue: "revenu du mois en cours",
    period7d: "7 jours",
    period30d: "30 jours",
    period3m: "3 mois",
    period12m: "12 mois",
    legendRevenue: "Revenus",
    legendCharges: "Charges",
    legendProfit: "Bénéfice net",
    noData: "Pas encore assez de données pour afficher un graphique.",
    // {pct} = pourcentage déjà formaté (ex. "12.5%" ou "+3.2%").
    summaryStableNone: "Le revenu est stable ce mois-ci (aucun revenu ce mois-ci ni le mois dernier).",
    summaryNew: "Le revenu est en hausse ce mois-ci (aucun revenu enregistré le mois dernier pour comparer).",
    summaryStable: "Le revenu est stable ce mois-ci ({pct} vs le mois dernier).",
    summaryUp: "Le revenu est en hausse de {pct} ce mois-ci par rapport au mois dernier.",
    summaryDown: "Le revenu est en baisse de {pct} ce mois-ci par rapport au mois dernier.",
  },
  // Calendrier d'occupation (components/reservations/reservation-calendar.tsx).
  calendar: {
    mon: "Lun",
    tue: "Mar",
    wed: "Mer",
    thu: "Jeu",
    fri: "Ven",
    sat: "Sam",
    sun: "Dim",
    clientFallback: "Client",
    reservationFallback: "Réservation",
  },
  propertyPerformance: {
    title: "Performance des propriétés",
    sortByPrefix: "Trier par",
    sortNetProfit: "Bénéfice net",
    sortRevenue: "Revenu",
    sortOccupancy: "Occupation",
    noProperties: "Aucune propriété enregistrée pour l'instant.",
    columnProperty: "Propriété",
    columnRevenue: "Revenu",
    columnCharges: "Charges",
    columnNetProfit: "Bénéfice net",
    columnOccupancy: "Occupation",
    unknownStatus: "Statut inconnu",
  },
  actionCenter: {
    title: "Centre d'actions",
    allDoneTitle: "Rien à faire aujourd'hui",
    allDoneSubtitle: "Aucune tâche en retard, aucune demande en attente.",
    andMoreOne: "autre",
    andMoreMany: "autres",
  },
  tools: {
    title: "Outils",
    properties: "Propriétés",
    reservationsCalendar: "Réservations (calendrier)",
    charges: "Charges",
    payments: "Paiements aux prestataires",
    tasks: "Tâches",
    exchangeRates: "Taux de change",
    financialReports: "Rapports financiers",
  },
  upcomingArrivals: {
    title: "Prochaines arrivées",
    none: "Aucune arrivée à venir.",
    unknownProperty: "Propriété inconnue",
    unknownClient: "Client inconnu",
    arrivingOn: "arrivée le",
  },
  allModules: {
    title: "Tous les modules",
    show: "Afficher",
    hide: "Masquer",
  },
  // Chrome fixe des cartes AI Property Assistant uniquement (titre, placeholder, bouton,
  // messages système) - le CONTENU généré par Gemini (insight du matin, réponses du chat)
  // reste en français, voir NOTES-ai-assistant.md / NOTES-multi-langue.md.
  assistant: {
    insightsLoading: "L'assistant prépare le résumé du jour...",
    insightsError: "L'assistant n'a pas pu générer les insights du jour.",
    retry: "Réessayer",
    chatTitle: "Pose une question à ton portefeuille",
    chatPlaceholder: "Ex : combien j'ai gagné ce mois-ci ?",
    chatButton: "Demander",
    chatHint:
      "Pose une question sur tes revenus, charges, réservations à venir ou tâches en retard — l'assistant répond uniquement à partir des vraies données de ton portefeuille.",
    chatError: "L'assistant n'a pas pu répondre pour le moment.",
  },
};

// Pas de "as const" ici : on veut que chaque feuille soit typée `string` (n'importe quel texte
// valide), pas la valeur littérale française exacte - "en" doit juste avoir les MÊMES CLÉS,
// pas les mêmes valeurs.
export type Dict = { [K in keyof typeof fr]: { [J in keyof (typeof fr)[K]]: string } };

const en: Dict = {
  common: {
    loading: "Loading...",
    cancel: "Cancel",
    close: "Close",
  },
  nav: {
    sectionOverview: "Overview",
    sectionOperations: "Operations",
    sectionFinances: "Finance",
    sectionTeam: "Team",
    dashboard: "Dashboard",
    properties: "Properties",
    reservations: "Reservations",
    reservationRequests: "Reservation requests",
    tasks: "Tasks",
    charges: "Expenses",
    payments: "Payments",
    financialReports: "Financial reports",
    exchangeRates: "Exchange rates",
    collaborators: "Collaborators",
    clients: "Clients",
    allModules: "All modules",
    defaultAgency: "Main agency",
    company: "Company",
    account: "Account",
    navigation: "Navigation",
    openMenu: "Open menu",
    expandMenu: "Expand menu",
    collapseMenu: "Collapse menu",
    expand: "Expand",
    collapse: "Collapse",
    logout: "Log out",
    home: "Home",
    search: "Search",
    notifications: "Notifications",
    breadcrumbProperty: "Properties & units",
    breadcrumbProfitability: "Profitability",
  },
  reserver: {
    heroTitle: "Our available properties",
    heroSubtitle: "Choose a property and send us your request — we'll get back to you quickly.",
    displayPricesIn: "Show prices in:",
    loadingProperties: "Loading...",
    noProperties: "No properties available at the moment.",
    perNight: "/ night",
    people: "people",
    viewPosition: "View location",
    request: "Request",
    requestForPrefix: "Request for:",
    requestSentTitle: "Request sent",
    thanksTitle: "Thank you!",
    requestSentBody: "Your request has been sent. We'll contact you shortly at",
    checkIn: "Check-in",
    checkOut: "Check-out",
    fullName: "Full name",
    phone: "Phone",
    messageOptional: "Message (optional)",
    minimumFieldsError: "Please provide at least your name and phone number.",
    genericSubmitError: "Error while sending",
    sendRequest: "Send request",
    sending: "Sending...",
  },
  login: {
    title: "Login",
    username: "Username",
    password: "Password",
    submit: "Sign in",
    submitting: "Signing in...",
    genericError: "Login failed",
    unrecognizedRoleError: "Unrecognized role for this user.",
    brandBadge: "Next-generation property management",
    brandTitleLine1: "All your management",
    brandTitleLine2: "in one place.",
    brandSubtitle:
      "Find your properties, reservations, charges, payments and teams in a space designed to simplify your day-to-day.",
    benefitMultiCompany: "Manage several companies from a single account",
    benefitIsolation: "Keep your data strictly partitioned",
    benefitRealtime: "Track your activity and profitability in real time",
    brandFooter: "A management space built for rental professionals.",
    backHome: "Back to home",
    backHomeShort: "Home",
    welcomeTitle: "Welcome",
    welcomeSubtitle: "Sign in to your DariRentals workspace to continue.",
    usernamePlaceholder: "Your username",
    passwordPlaceholder: "Your password",
    showPassword: "Show password",
    hidePassword: "Hide password",
    securityNote:
      "Your access is protected. Your workspace information stays separated by company and role.",
  },
  dashboardHeader: {
    title: "Dashboard",
    activeProperty: "active property",
    activeProperties: "active properties",
    account: "Account",
    logout: "Log out",
  },
  dashboardStats: {
    properties: "Properties",
    active: "active",
    reservations: "Reservations",
    upcoming: "upcoming",
    clients: "Clients",
    revenue: "Revenue (reservations)",
  },
  dashboardHome: {
    greeting: "Hello",
    greetingReturning: "glad to have you back.",
    subtitle: "Here's what's happening across your portfolio today.",
    subtitleCompanyPrefix: "Here's what's happening at",
    subtitleCompanySuffix: "today.",
    subtitleCompanyFallback: "your company",
    scanInvoice: "Scan an invoice",
    statRevenue: "Revenue",
    statRevenueHint: "Excluding cancelled reservations",
    statOccupancy: "Occupancy rate",
    statReservations: "Reservations",
    upcomingSuffix: "upcoming",
    statNetRevenue: "Net revenue",
    statNetRevenueHint: "Revenue − charges",
    todoTitle: "To do today",
    todoSubtitle: "Tasks that need your attention",
    todoEmpty: "Nothing urgent today.",
    taskFallback: "Task",
    seeAllTasks: "See all tasks",
    recentReservationsTitle: "Recent reservations",
    recentReservationsSubtitle: "The latest reservations for your properties",
    seeAll: "See all",
    colProperty: "Property",
    colClient: "Client",
    colDates: "Dates",
    colAmount: "Amount",
    colStatus: "Status",
    noReservations: "No reservations.",
    calendarTitle: "Calendar",
    calendarSubtitle: "Property occupancy",
  },
  propertyMap: {
    legendActive: "Currently occupied",
    legendEndingSoon: "Checking out within 3 days",
    legendIdle: "Vacant",
    viewProfitability: "View profitability",
    viewProperty: "View property",
    emptyTitle: "No properties located",
    emptySubtitle: "Add a latitude/longitude to your properties to see them here.",
  },
  dayTimeline: {
    title: "Today",
    subtitle: "Arrivals, departures and tasks for today",
    emptyTitle: "Nothing planned today",
    emptySubtitle: "Take a moment to breathe.",
    arrivalLabel: "Arrival",
    departureLabel: "Departure",
    taskLabel: "Task due",
  },
  healthScore: {
    title: "Portfolio health",
  },
  revenueIntelligence: {
    title: "Revenue",
    newBadge: "New",
    currentMonthRevenue: "current month revenue",
    period7d: "7 days",
    period30d: "30 days",
    period3m: "3 months",
    period12m: "12 months",
    legendRevenue: "Revenue",
    legendCharges: "Expenses",
    legendProfit: "Net profit",
    noData: "Not enough data yet to display a chart.",
    summaryStableNone: "Revenue is stable this month (no revenue this month or last month).",
    summaryNew: "Revenue is up this month (no revenue recorded last month to compare).",
    summaryStable: "Revenue is stable this month ({pct} vs last month).",
    summaryUp: "Revenue is up {pct} this month compared to last month.",
    summaryDown: "Revenue is down {pct} this month compared to last month.",
  },
  calendar: {
    mon: "Mon",
    tue: "Tue",
    wed: "Wed",
    thu: "Thu",
    fri: "Fri",
    sat: "Sat",
    sun: "Sun",
    clientFallback: "Client",
    reservationFallback: "Reservation",
  },
  propertyPerformance: {
    title: "Property performance",
    sortByPrefix: "Sort by",
    sortNetProfit: "Net profit",
    sortRevenue: "Revenue",
    sortOccupancy: "Occupancy",
    noProperties: "No properties registered yet.",
    columnProperty: "Property",
    columnRevenue: "Revenue",
    columnCharges: "Charges",
    columnNetProfit: "Net profit",
    columnOccupancy: "Occupancy",
    unknownStatus: "Unknown status",
  },
  actionCenter: {
    title: "Action center",
    allDoneTitle: "Nothing to do today",
    allDoneSubtitle: "No overdue tasks, no pending requests.",
    andMoreOne: "other",
    andMoreMany: "others",
  },
  tools: {
    title: "Tools",
    properties: "Properties",
    reservationsCalendar: "Reservations (calendar)",
    charges: "Charges",
    payments: "Payments to service providers",
    tasks: "Tasks",
    exchangeRates: "Exchange rates",
    financialReports: "Financial reports",
  },
  upcomingArrivals: {
    title: "Upcoming arrivals",
    none: "No upcoming arrivals.",
    unknownProperty: "Unknown property",
    unknownClient: "Unknown client",
    arrivingOn: "arriving on",
  },
  allModules: {
    title: "All modules",
    show: "Show",
    hide: "Hide",
  },
  assistant: {
    insightsLoading: "The assistant is preparing today's summary...",
    insightsError: "The assistant couldn't generate today's insights.",
    retry: "Retry",
    chatTitle: "Ask your portfolio a question",
    chatPlaceholder: "E.g.: how much did I earn this month?",
    chatButton: "Ask",
    chatHint:
      "Ask a question about your revenue, charges, upcoming reservations, or overdue tasks — the assistant only answers from your portfolio's real data.",
    chatError: "The assistant couldn't answer right now.",
  },
};

export const translations = { fr, en };
export type Locale = keyof typeof translations;
