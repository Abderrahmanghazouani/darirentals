...
import ma.zyn.app.service.impl.client.property.CountryClientServiceImpl;
...
@SpringBootTest
class CountryClientServiceImplTest {

    @Mock
    private CountryDao repository;
    private AutoCloseable autoCloseable;
    private CountryClientServiceImpl underTest;
    ...
    void setUp() {
        ...
        underTest = new CountryClientServiceImpl(repository);
    }
	...
	private Country constructSample(int i) {
        ...
        List<City> cities = IntStream.rangeClosed(1, 3)
                                             .mapToObj(id -> {
                                                City element = new City();
                                                element.setId((long)id);
                                                element.setName("name"+id);
                                                element.setCountry(new Country(Long.valueOf(2)));
                                                return element;
                                             })
                                             .collect(Collectors.toList());
        given.setCities(cities);
        return given;
    }
